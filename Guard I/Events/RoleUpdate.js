...
module.exports = async (oldRole, newRole) => {
    ...
    await client.channels.cache.find(x => x.name === "guard-log").send({embeds:[new MessageEmbed()
      .setTimestamp()
      .setFooter({text: "Rol İzinleri Düzenlendi"})
    .setDescription(`${entry.executor} (\`${entry.executor.id}\`) tarafından \`${role.name}\` \`${role.id}\` isimli rolü düzenledi ve geri düzeltilerek, yapan kişi yasaklandı`)
    .catch(`${entry.executor} (\`${entry.executor.id}\`) tarafından \`${role.name}\` \`${role.id}\` isimli rolü düzenlendi ve geri düzeltilerek, yapan kişi yasaklandı`)
  ]})
